...
class Item extends Model
{
    ...
    // Guarantee these are never null at the PHP level
    ...
    protected $fillable = [
        'code', 'name', 'description', 'category_id', 'type',
        'unit', 'unit_secondary', 'conversion_factor',
        'current_price', 'last_purchase_price', 'last_purchase_date', 'avg_price',
        'current_stock', 'min_stock_level', 'max_stock_level', 'reorder_level', 'lead_time_days',
        'is_consumable', 'is_asset', 'asset_threshold_amount',
        'brand', 'model', 'manufacturer', 'specifications',
        'photo', 'additional_photos', 'barcode', 'sku',
        'status', 'notes', 'attributes',
    ...
    protected static function boot()
    {
        ...
        static::saving(function ($item) {
            if (empty($item->type))   $item->type   = 'consumable';
    ...
    // Accessors — null-safe defaults prevent TypeError
    ...
    public function scopeConsumable($query)              { return $query->where('is_consumable', true); }
    public function scopeAsset($query)                   { return $query->where('is_asset', true); }
